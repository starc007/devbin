...
function isSystemPath(dir: string): boolean {
  const normalized = path.resolve(dir);
  return SYSTEM_PATH_PREFIXES.some((p) => normalized === p || normalized.startsWith(p + path.sep));
}

function isExecutableFile(dir: string, name: string): boolean {
...
export function listPath(options: {
  ...
  for (const dir of pathEnv.split(path.delimiter)) {
    ...
    if (!trimmed || isSystemPath(trimmed)) continue;
    try {
      ...
      for (const ent of entries) {
        ...
        if (!isExecutableFile(trimmed, name)) continue;
        seen.add(name);
